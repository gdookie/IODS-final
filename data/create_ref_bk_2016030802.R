...
# asylum_seekers <- read_delim("unhcr_asylum_seekers.csv", delim=",", skip=2)
# dim(asylum_seekers)
# glimpse(asylum_seekers)
# names(asylum_seekers) = c( "Year", "Country", "Origin", "RSD", "TotStatYear", "AppDYear", "DecRec", "DecOth", "Rejected", "OthClo", "TotDec", "TotEndY")
# names(asylum_seekers) 
# asylum_seekers$TotStatYear <- str_replace(asylum_seekers$TotStatYear, pattern=",", replace ="")
# asylum_seekers$TotStatYear <- as.numeric(asylum_seekers$TotStatYear)
# #asylum_seekers <- mutate(asylum_seekers, TotStatYear = TotStatYea
# asylum_seekers <- filter(asylum_seekers, !is.na(TotStatYear)) %>% group_by(Country, Origin, TotStatYear) %>% filter(Country) %>% summarise(csum = sum(TotStatYear)) %>% mutate(cTotal= csum) 
# head(asylum_seekers,20)
# 

# asylum_seekers$TotStatYear <- as.numeric(asylum_seekers$TotStatYear)
# asylum_seekers$TotStatYear 
# asylum_seekers <- asylum_seekers %>% group_by(Country) %>% summarise(sum((TotStatYear)))
